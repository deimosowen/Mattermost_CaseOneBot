...
const inReview = async ({ post_id, task_key, merge_request_url }) => {
    try {
        const post = await getPost(post_id);
        const user = await getUser(post.user_id);

        await reviewCommand({ post_id, channel_type: 'D', user_name: `@${user.username}`, args: [task_key, merge_request_url] });
